...
func init() {
	android.RegisterModuleType("nad_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = nadExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = nadExpandVariables(ctx, dep_file)
	...
	exCmd := nadExpandVariables(ctx, String(g.properties.Cmd))
